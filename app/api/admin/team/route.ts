...
import { defaultTeam } from "@/lib/cms/defaults";
import { teamSectionSchema } from "@/lib/cms/schemas";
...
export async function GET() {
  const settings = await prisma.teamSettings.findUnique({ where: { id: "default" } });
  if (!settings) {
  ...
  return NextResponse.json({ team: toPayload(settings) });
}
...
export async function PUT(request: Request) {
  ...
  const parsed = teamSectionSchema.safeParse(await request.json());
  if (!parsed.success) {
