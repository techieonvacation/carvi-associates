...
import type { WorkingProcessContent } from "@/lib/cms/types";
 ...
 */
export function WorkingProcess({ workingProcess }: WorkingProcessProps) {
  if (!workingProcess.isVisible || !workingProcess.steps.length) {
    return null;
  }
  ...
  return (
    <section className="working-process section-space bg-white py-30 max-md:py-25 max-sm:py-20">
      <Container>
        <SectionHeading
          align="center"
          tagline={workingProcess.tagline}
          lines={[...workingProcess.title]}
          taglineBg={workingProcess.taglineBg}
        />

        <div className="working-process__row grid grid-cols-1 gap-x-7.5 gap-y-14 md:grid-cols-2 lg:grid-cols-4 lg:gap-y-0">
          {workingProcess.steps.map((step, i) => {
            const isUp = i % 2 === 1;
            const isFirst = i === 0;
            const isLast = i === workingProcess.steps.length - 1;
            const href = step.href || "#";

            return (
              <Reveal key={step.id} direction="up" delay={i * 100} className="working-process__item">
                <div
                  className={cn(
                ...
                >
                  ...
                  <div className="working-process__content relative w-full">
                    ...
                    <h3 className="working-process__title mt-6 mb-2.5 text-[22px] leading-[1.272] font-bold text-foreground capitalize">
                      <Link href={href}>{step.title}</Link>
                    </h3>
